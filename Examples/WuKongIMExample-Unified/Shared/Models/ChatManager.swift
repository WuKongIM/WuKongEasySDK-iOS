...
"""
    ...
    @Published var recentRecipients: [RecipientInfo] = []

    /// Alert state for unsupported group channels
    @Published var showGroupUnsupportedAlert = false
    ...
    // MARK: - Recipient Management Methods

    /// Set the target recipient for messages
    func setRecipient(channelId: String, channelType: ChannelType) {
        // Prevent group channel selection for now
        if channelType == .group {
            showGroupUnsupportedAlert = true
            addLog("⚠️ Group channels are not yet supported")
            return
        }
    ...
    private func addToRecentRecipients(channelId: String, channelType: ChannelType) {
        let recipient = RecipientInfo(
            channelId: channelId,
            channelType: channelType,
            displayName: channelId, // In a real app, this would be fetched from user/group info
            lastUsed: Date()
        )

        // Remove if already exists
        recentRecipients.removeAll { $0.channelId == channelId && $0.channelType == channelType }

        // Add to beginning
        recentRecipients.insert(recipient, at: 0)

        // Keep only last 10 recipients
        if recentRecipients.count > 10 {
            recentRecipients = Array(recentRecipients.prefix(10))
        }
    }

    /// Get suggested recipients based on recent usage
    func getSuggestedRecipients(for channelType: ChannelType) -> [RecipientInfo] {
    ...
    func clearRecentRecipients() {
        recentRecipients.removeAll()
        addLog("Recent recipients cleared")
    }
}
